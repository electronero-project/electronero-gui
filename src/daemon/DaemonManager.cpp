...
bool DaemonManager::start(const QString &flags, NetworkType::Type nettype, const QString &dataDir, const QString &bootstrapNodeAddress)
{
    ...
    qDebug() << "starting pulsed " + m_pulsed;
    ...
    // Start monerod
    bool started = m_daemon->startDetached(m_pulsed, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM pulsed.exe");
#else
                QProcess::execute("pkill pulsed");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./pulsed status` returns BUSY when syncing.
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to pulsed
#ifdef Q_OS_WIN
    m_pulsed = QApplication::applicationDirPath() + "/pulsed.exe";
#elif defined(Q_OS_UNIX)
    m_pulsed = QApplication::applicationDirPath() + "/pulsed";
#endif

    if (m_pulsed.length() == 0) {
