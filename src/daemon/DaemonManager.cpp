...
bool DaemonManager::start(const QString &flags, NetworkType::Type nettype, const QString &dataDir, const QString &bootstrapNodeAddress)
{
    ...
    qDebug() << "starting electronerod " + m_monerod;
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM electronerod.exe");
#else
                QProcess::execute("pkill electronerod");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./electronerod status` returns BUSY when syncing.
